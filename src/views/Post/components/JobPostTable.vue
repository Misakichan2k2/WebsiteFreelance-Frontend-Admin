...
const savePost = async () => {
  try {
    ...
    const response = await onActionJobUpdate(updatedData);
    if (response && response.data) {
      ...
      await onActionJobPost().then((res) => {
...
const updateStatus = async (status, post) => {
  try {
    ...
    const response = await onActionJobStatusUpdate(updatedData); // Update status in the API
...
const deletePost = async (post) => {
  ...
  try {
    await onActionJobDelete(post._id); // Gửi yêu cầu xóa đến API
...
onMounted(async () => {
  ...
  await onActionJobPostCount().then((res) => {
...
<template>
  ...
  <div class="card">
    ...
    >
      ...
      <Column
        ...
        sortable
      ></Column>
      <Column header="Avatar" style="cursor: pointer">
        <template #body="{ data }">
  ...
  >
    ...
    <div class="flex align-items-center gap-3 mb-3">
      <label for="deadline" class="font-semibold w-6rem">Deadline</label>
      <DatePicker
        v-model="selectedPost.deadline"
        dateFormat="dd/mm/yy"
        class="flex-grow-1"
        showIcon
      />
    ...
    <div class="flex justify-content-between">
      <div class="flex align-items-center mb-3">
        <label for="recruitmentStatus" class="font-semibold w-7rem"
          >Recruitment Status</label
        >
        <Select
          v-model="selectedPost.recruitmentStatus"
      ...
      <div class="flex justify-content-between">
        <div class="flex align-items-center gap-3 mb-3">
          <label for="engRequired" class="font-semibold"
            >English Required</label
          >
          <Checkbox
            v-model="selectedPost.engRequired"
            binary
            class="flex-grow-1"
          />
        </div>
