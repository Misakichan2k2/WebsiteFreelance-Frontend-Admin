...
const fetchTransactions = async () => {
  try {
    const transactionsResponse = await onActionPendingTransaction();
    const countResponse = await onActionPendingTransactionCount();

    transactions.value = transactionsResponse.data.data;
    pendingTransactionCount.value = countResponse.data || 0;
...
const updateTransaction = async (data) => {
  ...
  confirm.require({
    ...
    accept: () =>
      onActionStatusUpdate("Paid", data).then(async () => {
        await fetchTransactions();
      }),
    reject: () =>
      onActionStatusUpdate("Rejected", data).then(async () => {
        await fetchTransactions();
      }),
  });
};
...
<template>
  ...
  <div class="card">
    ...
    >
      <template #header>
        <div class="flex justify-content-between -mx-2 pb-2">
          <div class="flex gap-2 align-items-center">
            <span class="font-semibold">All pending transactions:</span>
      ...
      <template #empty>
        <p class="text-center my-4">No pending transactions found.</p>
      </template>
      ...
      <Column header="Actions">
        <template #body="{ data }">
          <div class="flex gap-1">
            <Button @click="updateTransaction(data)" icon="pi pi-cog"></Button>
            <Button
              type="button"
